...
// templating-engine dependency -- same pattern as issuer-tool and
// holder-tool's src/web/layout.js.
...
const STYLE = `
  ...
  * { box-sizing: border-box; }
  ...
  header { background:linear-gradient(180deg,#0b3d30,#0f7a5c); color:#fff; padding:20px 28px; }
  ...
  .note-box { background:#eef1f3; border:1px solid var(--line); border-radius:10px; padding:12px 16px; font-size:13px; color:#3a4552; margin:14px 0; }
  .record-card { border:1.5px dashed var(--accent); border-radius:12px; padding:16px 18px; background:#f2faf7; margin-top:16px; }
  .summary-grid { display:flex; flex-wrap:wrap; gap:16px; margin-top:10px; }
  .summary-stat { background:#f2faf7; border-radius:10px; padding:10px 16px; min-width:110px; }
  .summary-stat .n { font-size:20px; font-weight:700; }
...
<title>${escapeHtml(title)} -- Common Credo Verifier</title>
<style>${STYLE}</style></head>
<body>
<header>
  <a href="/">Common Credo -- Check a Bundle</a>
</header>
<main>${body}</main>
</body></html>`;
}
